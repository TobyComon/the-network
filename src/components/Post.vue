<template>
   ...
   <img class="pfp selectable shadow" :src="post.creator.picture" alt="" @click="goToProfile">
        <h5 class=" mx-2 mt-2 text-capitalize" @click="goToProfile">{{post.creator.name}}
        </h5>
    ...
    <p class="text-center p-2"><strong>{{new Date(post.createdAt).toDateString()}}</strong></p>
    <p class="text-end me-3"><i class="text-danger selectable mdi mdi-heart mdi-24px" @click="likePost"></i> {{post.likes.length}}</p>
    </div>
</template>
...
export default {
    ...
    setup(props){
        ...
        return {
            ...
            goToProfile() {
               let active =  router.push({name: "Profile", params: {id: props.post.creatorId}})
